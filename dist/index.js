...
if (!STORAGE_ACCOUNT) {
    process.stderr.write("Fatal: AZURE_STORAGE_ACCOUNT environment variable is required.\n");
    process.exit(1);
}
const ACCOUNT_URL = `https://${STORAGE_ACCOUNT}.blob.core.windows.net`;
const ALLOWED_CONTAINERS = process.env
    .AZURE_STORAGE_ALLOWED_CONTAINERS
    ? new Set(process.env.AZURE_STORAGE_ALLOWED_CONTAINERS.split(",").map((s) => s.trim()))
...
const credential = new identity_1.ChainedTokenCredential(new identity_1.AzureCliCredential(), new identity_1.DefaultAzureCredential());
const blobServiceClient = new storage_blob_1.BlobServiceClient(ACCOUNT_URL, credential);
// ─── Guards ───────────────────────────────────────────────────────────────────
function assertContainerAllowed(container) {
    if (ALLOWED_CONTAINERS && !ALLOWED_CONTAINERS.has(container)) {
        throw new Error(`Container "${container}" is not in the allowed list. ` +
...
async function listContainers(prefix) {
    const results = [];
    for await (const container of blobServiceClient.listContainers({
        ...
        const c = container;
        const allowed = !ALLOWED_CONTAINERS || ALLOWED_CONTAINERS.has(c.name) ? "" : " [restricted]";
        const meta = c.metadata
            ? Object.entries(c.metadata)
                .map(([k, v]) => `${k}=${v}`)
                .join(", ")
            : "";
        results.push(`${c.name}${allowed}${meta ? `  (${meta})` : ""}`);
    }
    if (results.length === 0)
        return "No containers found.";
    return `Containers in ${STORAGE_ACCOUNT}:\n` + results.join("\n");
}
async function listBlobs(container, prefix, maxResults = 100) {
    ...
    if (blobs.length === 0)
        return `No blobs found in container "${container}".`;
    return (`Blobs in ${STORAGE_ACCOUNT}/${container}` +
        (prefix ? `/${prefix}` : "") +
        `:\n` +
        blobs.join("\n"));
}
async function readBlob(container, blobPath) {
    assertContainerAllowed(container);
    ...
    if (isBinary && size > 1048576) {
        return (`Blob "${blobPath}" appears to be binary (${contentType}, ${formatBytes(size)}). ` +
    ...
    const header = `Blob: ${STORAGE_ACCOUNT}/${container}/${blobPath}\n` +
        `Size: ${formatBytes(size)}  |  Type: ${contentType}  |  ` +
        `Modified: ${properties.lastModified?.toISOString() ?? "?"}\n\n`;
    return header + text;
}
async function writeBlob(container, blobPath, content, contentType = "text/plain; charset=utf-8", overwrite = true) {
    assertContainerAllowed(container);
    ...
    return (`Written ${formatBytes(buffer.length)} to ` +
        `${STORAGE_ACCOUNT}/${container}/${blobPath}  (${contentType})`);
}
async function deleteBlob(container, blobPath, deleteSnapshots = true) {
    assertContainerAllowed(container);
    const blobClient = blobServiceClient
        .getContainerClient(container)
        .getBlobClient(blobPath);
    ...
    return (`Deleted ${STORAGE_ACCOUNT}/${container}/${blobPath}\n` +
...
// ─── MCP Server ───────────────────────────────────────────────────────────────
const server = new index_js_1.Server({ name: "azure-blob", version: "1.0.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(types_js_1.ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "list_containers",
            description: "List all blob containers in the configured Azure Storage account. " +
        ...
        {
            name: "list_blobs",
            description: "List blobs inside a specific container. Optionally filter by path prefix " +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    prefix: {
                        type: "string",
                        description: "Optional path prefix to filter blobs (e.g. 'folder/subfolder/')",
                    },
                    max_results: {
                        type: "number",
        ...
        {
            name: "read_blob",
            description: "Download and return the text content of a blob. Suitable for text, " +
        ...
        {
            name: "write_blob",
            description: "Upload text content to a blob. Creates the blob if it does not exist, " +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    overwrite: {
                        type: "boolean",
                        description: "If false, the write will fail when the blob already exists (default: true)",
                    },
                },
        ...
        {
            name: "delete_blob",
            description: "Permanently delete a blob from a container. Also deletes any snapshots " +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    delete_snapshots: {
                        type: "boolean",
                        description: "Also delete snapshots of this blob (default: true)",
                    },
                },
                required: ["container", "blob_path"],
            },
    ...
    try {
        switch (name) {
            case "list_containers": {
                const result = await listContainers(args?.prefix);
            ...
            case "delete_blob": {
                const container = args?.container;
                const blobPath = args?.blob_path;
                const deleteSnapshots = args?.delete_snapshots ?? true;
                if (!container)
                    throw new Error("'container' is required");
                if (!blobPath)
                    throw new Error("'blob_path' is required");
...
async function main() {
    ...
    const restriction = ALLOWED_CONTAINERS
        ...
        : "all containers";
    process.stderr.write(`MCP azure-blob v1.0.0 — account: ${STORAGE_ACCOUNT} — access: ${restriction}\n`);
}
main().catch((err) => {
